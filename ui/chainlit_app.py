...
async def starters():
    return [
        cl.Starter(
            ...
            message=(
                "Summarize the document I uploaded — main points, "
                "structure, and anything notable."
            ),
        ),
        cl.Starter(
            label="Explain a figure",
            message=(
                "Explain the charts, diagrams or figures in this document "
                "and what they show."
            ),
        ),
        cl.Starter(
            ...
            message=(
                "Find every mention of deadlines, obligations or numbers "
                "in this document and cite the page."
            ),
        ),
    ]
    ...
    return [
        cl.ChatProfile(
            ...
            markdown_description=(
                "Searches your uploads **and** the shared corpus "
        ...
        cl.ChatProfile(
            ...
            markdown_description=(
                "Searches **only** documents you've uploaded this session "
                "— the shared corpus is excluded."
            ),
        ),
    ]
    ...
    await cl.Message(
        ...
        actions=[
            ...
            cl.Action(
                name="session_stats",
                value="click",
                label="📊 My session stats",
            ),
            cl.Action(
                name="clear_documents",
                value="click",
                label="🗑️ Clear my documents",
            ),
        ],
    ).send()
...
async def list_documents(action: cl.Action):
    ...
    if not docs:
        await cl.Message(
            content="You haven't uploaded any documents this session yet."
        ).send()
        return

    lines = [
        f"- `{d['filename']}` — {d['chunk_count']} chunks indexed"
        for d in docs
    ]

    await cl.Message(
        content="**Your uploaded documents:**\n" + "\n".join(lines)
    ).send()
...
async def session_stats(action: cl.Action):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.get(
            f"{API_URL}/admin/session-stats/{user_id}/{session_id}"
        )
...
async def clear_documents(action: cl.Action):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.post(
            f"{API_URL}/admin/cleanup/{user_id}/{session_id}"
        )
        resp.raise_for_status()
    ...
    await cl.Message(
        content=(
            "Cleared — your uploaded documents and vectors have been "
...
async def _upload_and_wait(
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        for _ in range(60):
            status = (
                await client.get(
                    f"{API_URL}/ingest/status/{doc_id}"
                )
            ).json()

            if status["status"] == "ready":
                status_msg.content = (
                    f"`{file_element.name}` ready — "
                    f"indexed {status['chunk_count']} chunks. "
            ...
            if status["status"] == "failed":
                status_msg.content = (
                    f"Failed to process `{file_element.name}`: "
                    f"{status.get('error')}"
                )
        ...
        status_msg.content = (
            f"Still processing `{file_element.name}` — "
            "try asking your question shortly."
        )
...
def _parse_sources(steps: list[dict]) -> list[cl.Text]:
    """
    Turns retrieved tool_result strings (already tagged
    '[doc_id p.page · type · source]') into individual Chainlit
    Text elements, so citations show as an expandable, clickable
    ...
    for step in steps:
        ...
        for match in _CITATION_RE.finditer(
            raw + "\n["
...
async def main(message: cl.Message):
    ...
    pdf_files = [
        ...
        if getattr(e, "name", "").lower().endswith(".pdf")
    ]
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        resp = await client.post(
            ...
            json={
                ...
                "user_id": user_id,
                "session_id": session_id,
                "include_shared": cl.user_session.get(
                    "include_shared"
                ),
    ...
    # Keep a short conversation history in the Chainlit session.
